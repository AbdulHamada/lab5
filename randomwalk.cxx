...
#include <fstream>
#include <iostream>
#include <sstream> //Characters can be inserted and/or extracted from the stream using any operation allowed on both input and output streams.
...
int main(void){
	...
	for(int j = 0; j < Nsubsteps; j++){
	   random( rx, ry,N); // call to function which randomly sets up rx and ry
	    ...
	    meany=0;
	    var=0;
	    statistic (c, rx, ry, N,  meanx,  meany, var);// call to function which evaluates statistics
...
void random(int* rx, int* ry, int N){
  for(int k=0; k<N; k++)
    {
	    rx[k]=int(3*(double (rand())/RAND_MAX))-1;// for steps
...
void statistic (colloid *const c,int *rx, int *ry, const int N, double & meanx, double & meany, double & var){
	...
	for(int i=0; i<N; i++){
	  ...
	  var=var1+var2;	 //var=pow(pow(var1,2)+pow(var2,2),0.5);   
}
// Ende of Programm>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>